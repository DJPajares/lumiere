...
2. Confirm the hero is readable immediately and every later section reveals whenever it re-enters
   from either edge while scrolling down and back up repeatedly.
...
All animated themes use an observer-backed reveal lifecycle so re-entry works consistently in both
scroll directions. Premium themes add an isolated animation-frame runtime for expressive depth and
short-lived image motion blur across browsers. It writes CSS variables from one scheduled animation
frame and never updates React state on scroll.
